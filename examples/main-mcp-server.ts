...
/**
 * Information about a processed coupon including verification results.
 */
interface ProcessedCouponInfo {
  coupon: Coupon;
...
const processedCoupons: Map<string, ProcessedCouponInfo> = new Map();
 ...
 */
async function verifyWithReputationServer(coupon: Coupon): Promise<CouponVerificationResult> {
  try {
    ...
    return result.verification as CouponVerificationResult;
  } catch (error) {
...
mcpServer.tool(
  'protected.getData',
  'Access protected data (requires valid coupon)',
  {
...
mcpServer.tool(
  ...
  'Manage the list of trusted issuers',
  {
...
mcpServer.tool(
  'checkProcessedCoupon',
  'Check if a coupon has been processed and its verification result',
  {
    couponId: z.string()
  },
  async (args) => {
...
mcpServer.tool(
  'protected.getReputationData',
  'Get reputation data for an entity (requires valid coupon)',
  {
