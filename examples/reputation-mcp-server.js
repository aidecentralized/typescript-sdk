 ...
 */
function updateReputation(result) {
 ...
 */
function updateEntityReputation(commonName, verified) {
...
mcpServer.tool('verifyCoupon', 'Verify a coupon and get reputation data', {
...
mcpServer.tool('getReputation', 'Get reputation data for an entity', {
    entity: z.string()
}, async (args) => {
...
mcpServer.tool('fetchAndVerifyCoupons', 'Fetch coupons from a server and verify them', {
    ...
    try {
        ...
        const coupons = data.map((item) => item.coupon); // Extract the coupon objects
...
mcpServer.tool('getTrustedIssuers', 'Get all trusted issuers', async () => {
    return {
        content: [
            {
                type: 'text',
...
mcpServer.tool('manageTrustedIssuers', 'Add or remove trusted issuers', {
